"""Interface-contract tests for the Week 2 submission (``estimator.py``).

Verifies the hard rules from ``week2.md`` §2 / §4.5:
...
``pytest`` is not required: running ``python test_interface.py`` executes every
check and prints a PASS/FAIL summary (exit code 1 on any failure).  The same
``test_*`` functions are collected normally if ``pytest`` is available.
"""
...
from estimator import estimate_latent_and_input  # noqa: E402
